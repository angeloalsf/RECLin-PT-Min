...
\title{RECLin-PT: o pré-treinamento clínico importa para a extração de
relações em notas clínicas em português? Um estudo controlado sobre o
SemClinBr}

\author{Angelo A. L. Silveira Filho\inst{1}, Cristiano da Silveira Colombo\inst{1}}

\address{Bacharelado em Sistemas de Informação --- Instituto Federal de
  Educação, Ciência e Tecnologia do Espírito Santo (IFES)\\
  Cachoeiro de Itapemirim --- ES --- Brasil
  \email{angelo.silveira@aluno.ifes.edu.br}\\
  \footnotesize\texttt{cristiano.colombo@ifes.edu.br}
}
...
relation between concepts, particularly the negation of findings. We
investigate whether \emph{clinical} pre-training is decisive for relation
extraction (RE) in Brazilian Portuguese over the SemClinBr corpus, in a
three-class space (negation, association and no-relation).
We fine-tune two Portuguese encoders under an identical pipeline---a
clinical-domain one (BioBERTpt) and a general-domain one (BERTimbau)---changing
only the pre-trained checkpoint. On the frozen test set, both models are
statistically tied on the \texttt{negation\_of} F1 (0.724 vs.\ 0.734; McNemar
$p=0.18$; paired bootstrap 95\% CI $[-0.047,+0.026]$), and the seed-to-seed
variation exceeds the gap between them. The general encoder matches the clinical
one with $\sim$60\% of its parameters, suggesting that, for this task, clinical
pre-training is not a decisive factor.
\end{abstract}

\begin{resumo}
O texto clínico livre concentra informação cuja interpretação depende da relação
entre conceitos, em especial a negação de achados. Este trabalho investiga se o
pré-treinamento \emph{clínico} é determinante para a extração de relações
(\emph{Relation Extraction}, RE) em português brasileiro sobre o corpus
SemClinBr, em um espaço de três classes (negação, associação e ausência de relação). Ajustamos dois \emph{encoders} para o português sob uma
mesma cadeia experimental---um de domínio clínico (BioBERTpt) e um de domínio
geral (BERTimbau)---variando apenas o \emph{checkpoint} de pré-treino. No
conjunto de teste congelado, os dois modelos empatam estatisticamente no F1 de
\texttt{negation\_of} (0,724 vs.\ 0,734; McNemar $p=0,18$; \emph{bootstrap}
pareado IC95\% $[-0,047;+0,026]$), e a variação entre sementes supera a diferença
entre eles. O \emph{encoder} geral iguala o clínico com $\sim$60\% dos
parâmetros, sugerindo que, para esta tarefa, o pré-treinamento clínico não é um
fator decisivo.
\end{resumo}
...
raramente testada de forma controlada para o português: estudos costumam adotar
um modelo clínico por hipótese, sem isolar o efeito do pré-treinamento de
domínio sob protocolo estatístico rigoroso.

Este trabalho responde diretamente à seguinte questão de pesquisa: \textbf{o
pré-treinamento clínico importa para a extração de relações em textos médicos em
português?} Para respondê-la, comparamos dois \emph{encoders} para o português
na \emph{mesma} tarefa de RE sobre o SemClinBr~\cite{oliveira:22}, mudando
\emph{somente} o \emph{checkpoint} de pré-treino: o BioBERTpt~\cite{schneider:20},
adaptado a narrativas clínicas e biomédicas em português, e o
BERTimbau~\cite{souza:20}, pré-treinado em texto de domínio geral (brWaC). Ambos
são ajustados sob uma única cadeia determinística, compartilhando a
representação de entrada, a função de perda, o escalonador, a seleção de modelo,
as métricas e a semente, de modo que qualquer diferença observada possa ser
atribuída ao pré-treinamento, e não a artefatos de implementação.

As contribuições deste artigo são: (i) um protocolo experimental reprodutível e
controlado---do texto bruto às métricas finais---que isola o efeito do
pré-treinamento de domínio na RE clínica em português; (ii) uma comparação
empírica, com teste de significância pareado, entre um \emph{encoder} clínico e
um geral sobre o SemClinBr; e (iii) a evidência de que, neste cenário, o
\emph{encoder} de domínio geral iguala estatisticamente o clínico, inclusive na
classe de negação, com cerca de 60\% dos parâmetros. O restante do artigo está
organizado da seguinte forma: a Seção~\ref{sec:fund} apresenta a fundamentação;
a Seção~\ref{sec:rel} discute trabalhos relacionados; a Seção~\ref{sec:met}
descreve materiais e métodos; a Seção~\ref{sec:exp} reporta os experimentos e
resultados; a Seção~\ref{sec:disc} traz a discussão; e a Seção~\ref{sec:conc}
conclui.
...
NER e adota um modelo clínico por hipótese, este trabalho aborda a etapa
subsequente de RE e, sobretudo, \emph{testa} de forma controlada se o ganho do
pré-treinamento clínico observado em NER se transfere para a extração de
relações, com atenção particular à negação clínica.

\section{Materiais e Métodos}\label{sec:met}

A premissa metodológica central é a \emph{paridade}: os dois modelos comparados
compartilham um único núcleo de implementação e diferem apenas no
\emph{checkpoint} de pré-treino. Todas as etapas foram projetadas sob
reprodutibilidade: ordenação determinística das saídas, serialização fixa e
semente aleatória registrada (42).
...
compartilhada por ambos os modelos---condição necessária para que os resultados
fiquem na mesma régua estatística. Para cada documento, percorrem-se todos os
pares ordenados de entidades $(e_1, e_2)$, $e_1 \neq e_2$ (a ordem importa,
porque a direção é relevante para a negação); descartam-se os pares cuja
...
documentado para essa classe. Como ambos os modelos recebem exatamente os mesmos
candidatos, o teto é idêntico para os dois e não afeta a comparação.
...
\texttt{CHEM} (químicos e fármacos), \texttt{PROC} (procedimentos)---, acrescidos
de duas camadas próprias do corpus que carregam sinal relevante para a tarefa:
\texttt{Abbreviation} e \texttt{Negation} (esta diretamente associada à classe
\texttt{negation\_of}). Como a mesma representação de entrada é aplicada aos dois
modelos, qualquer efeito dessa escolha é compartilhado e não confunde a
comparação.

\subsection{Modelos comparados e paridade}
Comparam-se dois \emph{encoders} para o português sob a mesma tarefa, variando
apenas o \emph{checkpoint} de pré-treino:
\begin{itemize}
...
desenvolvimento; e a semente de reprodutibilidade (42). Trocar um modelo pelo
outro consiste apenas em mudar o \emph{checkpoint} carregado.
...
A comparação entre os dois modelos emprega dois testes complementares,
...
\texttt{associated\_with} e 151 \texttt{negation\_of}). A
Tabela~\ref{tab:resultados} consolida os resultados dos dois \emph{encoders} na
semente 42. Os melhores valores por coluna estão em negrito.

\begin{table}[ht]
\centering
\caption{Resultados no conjunto de teste (semente 42). Negrito indica o melhor
valor por coluna.}
\label{tab:resultados}
\begin{tabular}{lcccccc}
\toprule
\textbf{Modelo} & \textbf{Macro-F1} & \textbf{F1 neg.} & \textbf{F1 assoc.} &
...
\subsection{Desempenho dos dois \emph{encoders}}
Os dois modelos apresentam desempenho praticamente indistinguível. O BioBERTpt
clínico atinge macro-F1 de 0,707 e o BERTimbau geral, 0,704---uma diferença de
0,003. Na métrica-alvo, o F1 de \texttt{negation\_of}, a ordem inclusive se
inverte: o BERTimbau geral (0,734) supera marginalmente o BioBERTpt clínico
(0,724). As demais métricas seguem o mesmo padrão de quase empate: ambos
...
dos dois modelos, normalizadas por linha (cada célula é a fração das instâncias
de uma classe verdadeira atribuída a cada classe predita; a diagonal corresponde
ao \emph{recall} por classe). Os dois padrões de erro são notavelmente
semelhantes: diagonal forte na negação e na classe majoritária, e o erro
dominante remanescente na confusão entre \texttt{associated\_with} e
...
modelos no conjunto de teste. No F1 de \texttt{negation\_of}, a diferença
observada é de apenas $-0,010$ (a favor do BERTimbau geral). O teste de McNemar
não rejeita a hipótese nula de igualdade ($p=0,18$): das 946 instâncias em que os
modelos discordam, 494 favorecem o BioBERTpt e 452 o BERTimbau, uma assimetria
compatível com o acaso. O \emph{bootstrap} pareado confirma o resultado: o
intervalo de confiança de 95\% para a diferença no F1 de \texttt{negation\_of} é
$[-0,047;+0,026]$, que \emph{inclui o zero}, com valor-$p$ de 0,57. Em ambos os
testes, portanto, \textbf{não há diferença estatisticamente significativa} entre
o \emph{encoder} clínico e o geral.
...
Para situar a magnitude da diferença entre os modelos, treinamos o BioBERTpt com
uma segunda semente (43). O macro-F1 cai de 0,707 (semente 42) para 0,665
(semente 43), e o F1 de \texttt{negation\_of}, de 0,724 para 0,653---uma variação
de 0,071 atribuível \emph{apenas} à inicialização aleatória do treino. Essa
variação entre sementes de um \emph{mesmo} modelo é cerca de sete vezes maior que
a diferença de 0,010 observada entre o BioBERTpt e o BERTimbau na semente 42.
Em outras palavras, o ruído de treino do encoder clínico, isoladamente, é maior
que a vantagem que se poderia atribuir ao domínio de pré-treino, o que reforça a
conclusão de empate.
...
domínio, e essa estrutura é fornecida igualmente aos dois modelos pela
representação de entrada compartilhada.

Um corolário prático relevante é a eficiência: o BERTimbau iguala o BioBERTpt
com aproximadamente 60\% dos parâmetros (108,9 vs.\ 177,9 milhões). Quando o
desempenho é equivalente, o modelo menor é preferível por custo de inferência e
de memória---uma consideração concreta para implantação em ambientes clínicos
...
implementação entre os dois modelos garante que a diferença medida reflita o
\emph{checkpoint}, e não artefatos. Persiste, porém, uma decisão de projeto: a
janela \texttt{max\_gap} na geração de pares descarta relações verdadeiras mais
distantes, impondo um teto de \emph{recall} à classe \texttt{associated\_with};
como esse teto é idêntico para os dois modelos, ele não afeta a comparação, mas
limita o desempenho absoluto reportado. A conclusão de empate apoia-se em uma
única semente para o BERTimbau; a análise de robustez por semente foi conduzida
apenas para o BioBERTpt e mostra que a variação de inicialização supera a
diferença entre modelos---o que sustenta, e não enfraquece, a leitura de empate,
mas uma estimativa multi-semente para ambos os modelos a tornaria mais
definitiva.
...
modelos maiores ou em cenários com menos dados de ajuste.

\section{Conclusão}\label{sec:conc}

Este trabalho investigou, de forma controlada, se o pré-treinamento clínico
importa para a extração de relações entre entidades clínicas em português sobre o
corpus SemClinBr, no espaço de rótulos \texttt{negation\_of},
\texttt{associated\_with} e \texttt{no\_relation}. Comparando um \emph{encoder}
clínico (BioBERTpt) e um geral (BERTimbau) sob uma cadeia experimental idêntica
que varia apenas o \emph{checkpoint} de pré-treino, encontramos um empate
estatístico: a diferença no F1 de \texttt{negation\_of} é de 0,010 e não é
...
A principal contribuição é, assim, metodológica e empírica: um protocolo
reprodutível que isola o efeito do domínio de pré-treino na RE clínica em
português, e a evidência de que, para esta tarefa, esse efeito não é decisivo.
Como trabalhos futuros, prevê-se: estimar o desempenho de ambos os modelos sob
múltiplas sementes de treino, para um intervalo de confiança da diferença mais
estreito; estender o espaço de relações para além das três classes atuais;
avaliar modelos maiores e \emph{encoders} multilíngues~\cite{conneau:20}; e
integrar uma etapa de NER automática, medindo a RE em um cenário fim a fim.
